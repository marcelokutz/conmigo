...
if ( ! class_exists( 'WPEX_WooCommerce_Config' ) ) {
	...
	class WPEX_WooCommerce_Config {
		...
		public function __construct() {
			...
			if ( wpex_get_mod( 'woo_dynamic_image_resizing', true ) ) {
		...
		public static function custom_cart_icon_css( $css ) {
			...
			if ( $icon_class && 'shopping-cart' != $icon_class ) {
				...
				if ( $icon_content ) {
					$css .= '.woocommerce .woocommerce-info a.button.wc-forward:before, .woocommerce .woocommerce-message a.button.wc-forward:before { content: "' . $icon_content . '"; }';
				}
			}
			return $css;
		}
